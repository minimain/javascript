...
//그림이 안보이는 이유는 높이값을 css에서 다 지워버림
...
//window안에 있는 현재 크기의 화면 높이값이 devHeight라고 지정
// 여기까지는 그 높이값을 지정할려면 새로고침을 해줘야함

// 이건 새로고침을 안해도 크기가 변경됨 높이
...
for(let section of sections){
  section.addEventListener("wheel", e =>{ //scroll up (-)음의 숫자
    if(e.deltaY < 0){//scroll up (-)음의 숫자 | 0이 높이값보다 크다
     let prev = e.currentTarget.previousElementSibling.offsetTop;
    //  previousElementSibling : 선택된 요소의 전 요소를 선택 그래서 여기서는 이전 회면으로 이동
    window.scroll({
      top: prev,
      left: 0,
      behavior: "smooth"
